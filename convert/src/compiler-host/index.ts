...
export const createCompilerHostFromFile = (filePath: string, rootDirectory: string = process.cwd()) => {
  ...
  let compilerOptions = { strict: true, target: ts.ScriptTarget.Latest, allowJs: true, module: ts.ModuleKind.Node12 } as ts.CompilerOptions;
  if (existsSync(tsconfigPath)) {
    compilerOptions = JSON.parse(readFileSync(tsconfigPath).toString("utf-8"));
  }
