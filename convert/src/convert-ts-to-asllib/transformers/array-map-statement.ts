...
export const arrayMapTransformer = <T extends ts.Node>(context: ts.TransformationContext) => (rootNode: T) => {
  function visit(node: ts.Node): ts.Node {
    ...
    if (ts.isCallExpression(node)) {
      if (ts.isPropertyAccessExpression(node.expression) && node.expression.name.text === "map") {
        ...
        const name = TransformUtil.createNamePropertyAssignment(node, "For %s Of %s", arg.parameters[0].name, node.expression);
