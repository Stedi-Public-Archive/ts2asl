...
export const unsupportedStatementTransformer = (converterOptions: ConverterOptions) => <T extends ts.Node>(context: ts.TransformationContext) => (rootNode: T) => {
  function visit(node: ts.Node): ts.Node {
    ...
    if (ts.isSpreadAssignment(node)) throw new ParserError(`spread assignment is not support, see: https://github.com/OlafConijn/ts2asl/issues/36`, node);
    if (ts.isObjectBindingPattern(node)) throw new ParserError(`object binding is not support, see: https://github.com/OlafConijn/ts2asl/issues/37`, node);
