...
export const executeStepFunction = async (fixture: string, name: string, input: {} = {}): Promise<unknown> => {
  ...
  try {
    ...
    const execution = await asl.sdk(SFN).startExecution({ parameters: { stateMachineArn: resource.PhysicalResourceId, input: JSON.stringify(input, null, 2) } });
    let status = "RUNNING";
    let output : string | undefined;
    do{ 
      await sleep(1000);
      const result = await asl.sdk(SFN).describeExecution({ parameters: {executionArn: execution.executionArn} });
